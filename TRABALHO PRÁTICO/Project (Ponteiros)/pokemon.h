...
void pokemon_player1(FILE* entrada, struct pokemon* player, int opcao) {
    ...
    // Ler os atributos do pokemon e colocá-los na estrutura struct pokemon p1:
...
void pokemon_player2(FILE* entrada, struct pokemon* player, int opcao) {
    ...
    // Ler os atributos do pokemon e colocá-los na estrutura struct pokemon p2:
...
void ataque(struct pokemon* atacante, struct pokemon* defensor) {
    ...
    if (compara_tipos(atacante, defensor) == 0) {
        ...
        ataque += atacante->ataque*0.2;
        if (ataque > defensor->defesa /*+1*/) {
            defensor->vida -= (ataque - defensor->defesa);
        } else {
            defensor->vida -= 1.0;
        }
